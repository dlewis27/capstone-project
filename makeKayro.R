...
plot2List = function(inList, inList2){
  
  data(genesymbol, package = "biovizBase")
  tryList = getList(inList)
  tryList2 = getList(inList2)
  wh <- genesymbol[tryList]
  wh2 = genesymbol[tryList2]
  #wh <- range(wh)
  kp <- plotKaryotype(genome="hg19",plot.type=1)
  #chromosomes=c("chr8"))
  #chromosomes=c("chr1", "chr3", "chr7", "chr8", "chr20"))
  kpPlotRegions(kp, wh, col="orange")
  kpPlotRegions(kp, wh2, co ='lightblue')
}
...
#plot the 400 genes and the 32
plot2List(genes400$genes, myList)
